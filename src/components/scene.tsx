...
const Scene = ({ assets, hasInteracted, startIntroAnimation, onIntroAnimationComplete, setViewState, viewState, isLoginDialogOpen, setReturnHandler }: SceneProps) => {
    ...
    useEffect(() => {
        ...
        if (target === 'register' && hasInteracted && !showContentContainer && !isLoginDialogOpen && modelRef.current) {
            const blueBall = modelRef.current.getObjectByName('blueBall');
            const targetWaypoint = waypointsRef.current['blueBall'];

            if (blueBall && targetWaypoint && cameraRef.current && controlsRef.current) {
                // Trigger Spark Video
                const sparkMaterial = sparkMaterialsRef.current['Spark_2']; // blueBall corresponds to Spark_2
                if (sparkMaterial && assets.sparkVideo) {
                    sparkMaterial.visible = true;
                    assets.sparkVideo.currentTime = 0;
                    assets.sparkVideo.play().catch(e => console.error("Video play failed:", e));
                }
                ...
                setZoomedTarget(blueBall as THREE.Mesh);
            }
        }
    }, [searchParams, hasInteracted, showContentContainer, isLoginDialogOpen, setViewState, assets.sparkVideo]);
    ...
    return (
        <>
            ...
            <div
                className={`hidden md:block absolute top-[5.75rem] right-10 w-[320px] max-h-[50vh] transition-opacity duration-300 pointer-events-none transform origin-top-right scale-[0.64] ${hoveredLabel ? 'opacity-100' : 'opacity-0'
                    }`}
            >
