...
export function MidiPlayer({ midi, currentTime, playbackSpeed, onClose }: MidiPlayerProps) {
  ...
  const totalDuration = midi.duration / playbackSpeed;
  const pixelsPerSecond = 150 * playbackSpeed; 
  ...
  const effectiveCurrentTime = currentTime * playbackSpeed;

  return (
    <div 
      className={cn(
        "fixed z-20 h-64 bg-black/50 backdrop-blur-md border border-orange-500/20 rounded-lg w-full max-w-[1010px] transition-all duration-500 ease-in-out scale-50",
        "bottom-4",
    ...
    >
      ...
      <Button 
        ...
        onClick={onClose}
        className={cn(
            "absolute top-2 right-2 z-30 bg-black/30 hover:bg-black/60 text-orange-200 transition-all duration-500 ease-in-out backdrop-blur-md rounded-full h-8 w-8"
        )}
      >
        <X className="h-4 w-4" />
      </Button>
      ...
      <div className="w-full h-full relative overflow-y-auto overflow-x-hidden rounded-lg flex justify-center">
        <svg
          className="absolute top-0 left-1/2 -translate-x-1/2"
          width={totalWidth}
          height={viewboxHeight + KEYBOARD_HEIGHT}
          style={{ transform: `translateY(-${Math.max(0, viewboxHeight - (effectiveCurrentTime * pixelsPerSecond) - (256-KEYBOARD_HEIGHT) )}px) translateX(-50%)`}}
        >
            <defs>
          ...
          {notes.map((note, i) => {
            ...
            const noteTime = note.time / playbackSpeed;
            const noteDuration = note.duration / playbackSpeed;
            
            const y = (totalDuration - (noteTime + noteDuration)) * pixelsPerSecond;
            const height = noteDuration * pixelsPerSecond;
            const isPlaying = currentTime >= noteTime && currentTime < noteTime + noteDuration;
       ...
       <div className="absolute bottom-0 left-0 right-0 h-[100px] z-10 pointer-events-none rounded-b-lg overflow-hidden flex justify-center">
          <svg width={totalWidth} height={KEYBOARD_HEIGHT} className="absolute bottom-0 left-1/2 -translate-x-1/2">
            {/* Piano Roll Keys */}
